...
import {
  ...
  currentPageSetItem,
  currentPageGetItem,
  chosenProductIdSetItem,
  shoppingCartSetItem,
} from "./components/dataHandeling/handeling.js";
...
export function handleAddToCart(
  ...
  setTimeout(() => {
    ...
    button.classList.remove("main-color");
...
export function removeFromCart(chosenID) {
  model.input.cart = model.input.cart
    .map((item) => {
      if (item.id === chosenID) {
        if (item.amount > 1) {
...
export function checkoutHandeling() {
  ...
  const generateOrderID =
    Math.random() * (model.data.orderHistory.length + 5) * 55;
...
export function calculateTotal(list) {
  ...
  list.forEach((element) => {
    amount += element.onSale
      ? element.discountedPrice * element.amount
...
export function findRecentOrder() {
  orderHistoryGetItem();
  orderIDGetItem();
  const recentOrder = model.data.orderHistory.find(
    (item) => item.orderID == model.input.recentOrder,
  );
  return recentOrder;
}
